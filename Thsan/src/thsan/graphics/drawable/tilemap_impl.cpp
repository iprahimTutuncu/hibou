...
namespace Thsan {
    ...
    bool TilemapImpl::load(const int* data) {
        ...
        mesh = Mesh::create(totalTiles * 4);

        std::vector<Vertex> vertices;
        vertices.reserve(totalTiles * 4);
        glm::vec4 color = tsm::Color(255, 255, 255, 255).toGlm();
        ...
        for (unsigned int i = 0; i < tilemapWidth; ++i)
            for (unsigned int j = 0; j < tilemapHeight; ++j)
            {
                ...
                glm::vec2 texCoords = tileset_coordinates[tileData[i]];
                glm::vec2 texCoordsSize(tileWidth / image_w, tileHeight / image_h);

                if (tileData[i] == 0) {
                    texCoords = glm::vec2(0.f, 0.f);
                    texCoordsSize = glm::vec2(0.f, 0.f);
                }
                ...
                vertices.push_back(Vertex(glm::vec3(i * tw, j * th, 0.f), glm::vec3(0.f, 0.f, -1.f), texCoords));
                vertices.back().color = color;

                vertices.push_back(Vertex(glm::vec3((i + 1) * tw, j * th, 0.f), glm::vec3(0.f, 0.f, -1.f), glm::vec2(texCoords.x + texCoordsSize.x, texCoords.y)));
                vertices.back().color = color;

                vertices.push_back(Vertex(glm::vec3((i + 1) * tw, (j + 1) * th, 0.0f), glm::vec3(0.f, 0.f, -1.f), glm::vec2(texCoords.x + texCoordsSize.x, texCoords.y + texCoordsSize.y)));
                vertices.back().color = color;

                vertices.push_back(Vertex(glm::vec3(i * tw, (j + 1) * th, 0.0f), glm::vec3(0.f, 0.f, -1.f), glm::vec2(texCoords.x, texCoords.y + texCoordsSize.y)));
                vertices.back().color = color;
    ...
    void TilemapImpl::setTileAt(unsigned int x, unsigned int y, int tileID) {
        ...
        if (x < tilemapWidth && y < tilemapHeight) {
            ...
            glm::vec2 texCoords = tileset_coordinates[tileID];
            glm::vec2 texCoordsSize(tw / image_w, th / image_h);
            ...
            (*mesh)[index].texCoord = glm::vec2{ texCoords.x + texCoordsSize.x, texCoords.y };
            (*mesh)[index].texCoord = glm::vec2{ texCoords.x + texCoordsSize.x, texCoords.y + texCoordsSize.y };
            (*mesh)[index].texCoord = glm::vec2{ texCoords.x, texCoords.y + texCoordsSize.y };
    ...
    tsm::Color TilemapImpl::getTileColorAt(unsigned int x, unsigned int y) const {
        ...
        if (x < tilemapWidth && y < tilemapHeight) {
            ...
            if (meshIndex >= 0 && meshIndex < mesh->getVertexCount())
                return tsm::Color::fromGlm((*mesh)[meshIndex].color);
            else {
    ...
    void TilemapImpl::setTileColorAt(unsigned int x, unsigned int y, tsm::Color color) {
        ...
        if (x < tilemapWidth && y < tilemapHeight) {
            ...
            if (meshIndex >= 0 && meshIndex < mesh->getVertexCount()) {
                for (int i = 0; i < 4; i++)
                    (*mesh)[meshIndex + i].color = color.toGlm();
    ...
    bool TilemapImpl::loadTileset(Tileset tileset, std::shared_ptr<Texture2D> tileset_texture)
    {
        ...
        for (int y = 0; y < rows; y++) {
            for (int x = 0; x < columns; x++) {
                ...
                tileset_coordinates[++tileIndex] = glm::vec2(norm_start_x, norm_start_y);
            }
        }
    ...
    void TilemapImpl::draw(const RenderTarget& target, RenderStates& states) const
    {
        states.setTexture(TextureType::Default, tileset_texture);
    ...
    glm::mat4 TilemapImpl::getTransform()
    {
