...
static char* combinePath(const char* dir, const char* fname, bool isFolder)
{
	...
	size_t total_len = dir_len + fname_len + 2 + (isFolder ? 1 : 0);

	char* out = (char*)IM_ALLOC(total_len);
	IM_ASSERT(out);

	strcpy_s(out, total_len, dir);

	if (dir_len > 0 && dir[dir_len - 1] != '/')
		strcat_s(out, total_len, "/");

	strcat_s(out, total_len, fname);
