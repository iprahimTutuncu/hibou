...
public:
    ...
    void input(const float& deltaTime, std::vector<Config::InputAction> inputActions) override
    {
    ...
    void draw(sf::RenderTarget& target, const float& deltaTime)
    {
...
private:
    ...
    void load()
    {
        ...
        if (curr != next) {
            next->init();
            next->update(float::Zero);
            sf::RenderTexture temp_rt;
            temp_rt.create(1, 1);
            next->draw(temp_rt, float::Zero);
        }
