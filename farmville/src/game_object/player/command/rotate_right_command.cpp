#include "rotate_right_command.h"
...
void RotateRightCommand::execute(GameObject& parent, Scene& scene, const float& dt)
{
...
void RotateRightCommand::undo(GameObject& parent, Scene& scene, const float& dt)
{
	GameObject* go = scene.get_child("camera");
	CameraData* camera_data = go->getComponent<CameraData>(DataType::CAMERA);
	PlayerData* player_data = parent.getComponent<PlayerData>(DataType::PLAYER);
