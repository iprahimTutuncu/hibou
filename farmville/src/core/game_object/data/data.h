...
struct BagData : public Data {
    BagData() {
    }

    void addPlant(PlantData planData) {
        auto iter = std::find_if(plants.begin(), plants.end(), [&](const PlantData& plant) {return plant.getId() == planData.getId(); });
        if (iter == plants.end())
            plants.push_back(planData);
    }
    void addItem(const std::string& name, const std::string& desc) {
        if (items.count(name) > 0)
            items[name].second++;
        else {
            items[name].first = Item{ name, desc };
            items[name].second = 1;
        }
    }
    void addGear(const std::string& name, const std::string& desc) {
        if (gears.count(name) > 0)
            gears[name].second++;
        else {
            gears[name].first = Gear{ name, desc };
            gears[name].second = 1;
        }
    }

    void removePlant(const std::string& name_id) {
        auto iter = std::find_if(plants.begin(), plants.end(), [&name_id](const PlantData& plant) {return plant.getId() == name_id; });
        if (iter != plants.end())
            plants.erase(iter);
    }
    void removeOneItem(const std::string& name) {
        if (items.count(name) >= 2)
            items[name].second--;
        else {
            items.erase(name);
        }
    ...
    std::vector< std::pair<Item, int>> getItems() {
        std::vector< std::pair<Item, int>> v_items;
        for (const auto& elem : items) {
            v_items.push_back(elem.second);
        }
        return v_items;
    }

    std::vector<std::pair<Gear, int>> getGears() {
        std::vector< std::pair<Gear, int>> v_gears;
        for (const auto& elem : gears) {
            v_gears.push_back(elem.second);
        }
        return v_gears;
    }
