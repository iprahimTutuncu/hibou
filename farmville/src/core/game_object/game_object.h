...
class GameObject {
public:
    GameObject(Scene& scene, InputComponent* inputComp, PhysicComponent* physicComp,
        RenderComponent* renderComp, std::string name = "None");

    GameObject(const GameObject& other);
    ~GameObject();

    void updateInput(Scene& scene, const float& delta, const std::vector<Thsan::InputAction>& inputActions);
    ...
    template<typename T>
    T* getComponent(DataType type) {
        return m_componentDataHolder.getComponent<T>(type);
    }

    template<typename T>
    T getCopyComponent(DataType type) {
        return *(m_componentDataHolder.getComponent<T>(type));
    }
    ...
    void setNativeScript() {
        InstantiateScript = []() {
            return static_cast<NativeScriptComponent*>(new T());
            };
        ownScript = true;
    }
